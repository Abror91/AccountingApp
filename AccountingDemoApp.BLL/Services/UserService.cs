...
namespace AccountingDemoApp.BLL.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<OperationDetails> DepositMoney(decimal? amount, string userId)
        {
            if (amount == null)
                return new OperationDetails(false, "Укажите сумму дахода!", "");
            if (string.IsNullOrWhiteSpace(userId))
                return new OperationDetails(false, "Пользователь не гайден!", "");
            if (amount <= 0)
                return new OperationDetails(false, "Сумма дахода не валидна!", "");
            var user = await _db.UserManager.FindByIdAsync(userId);
            if(user == null)
                return new OperationDetails(false, "Пользователь не найден!", "");
            user.Deposit += (decimal)amount;
            await _db.SaveChanges();
            return new OperationDetails(true, "Ваш депозит успешно пополнен!", "");
        }
